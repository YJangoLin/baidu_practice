...
class StoneBaseDump:
    ...
    def _should_pull(self):
        # todo：修改
        file = f"{self.savePath}/es_origin_data_{self.sdate[5:7] + self.sdate[8:10]+self.sdate[11:19]}.json"
...
class BaseClientHolder:
    ...
    def __init__(self, date, savePath):
        self.date = date
        self.savePath = savePath
    ...
    def _split(self, days=1):
        ...
        if len(self.date) == 10:
            ...
            dateFormat = "%Y-%m-%d %H"
        start_date = datetime.datetime.strptime(self.date, dateFormat)
...
class ESClientHolder(BaseClientHolder):
    ...
    def __init__(self, date, savePath):
        super(ESClientHolder, self).__init__(date, savePath)
...
class Dump(StoneBaseDump):
    ...
    def __init__(self, sdate, edate, savePath, qlist):
        # !: 主要观察index是_还是-
        super(Dump, self).__init__(sdate, edate, savePath, index="shc_v1*")
        self.qlist = qlist
