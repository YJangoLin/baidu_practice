...
class BaseClientHolder:
    ...
    def _split(self, days=1):
        ...
        # bins = pd.date_range(
        #     start=sdate, end=edate, freq=f"{24 * 60 / 1}min"
        # ).astype(str)
...
class Dump(StoneBaseDump):
    ...
    def __init__(self, sdate, edate, savePath, qlist):
        # !: 主要观察index是_还是-
        super(Dump, self).__init__(sdate, edate, savePath, index="shc_preonline*")
        self.qlist = qlist
        ...
        return {
            ...
            "query": {
                "bool": {
                    ...
                    "must": [
                        {"match_phrase": {"message": "match_type"}},
                        {"match_phrase": {"message": "elapse_time"}},
                        {"match_phrase": {"message": "10qps_not_sleep"}}
                    ],
                }
            },
        }
